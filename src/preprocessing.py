...
def load_data(file_path):
    # Skip metadata rows 1 and 2, first row is header (columns)
    df = pd.read_csv(file_path, skiprows=[1,2], header=0, index_col=0, parse_dates=True)
    # Now index is the Date (parsed as datetime)
    return df
